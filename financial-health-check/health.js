/* terencana.id — Financial Health Check Wizard (hybrid) */
...
//   saveState();
//   btnModeCards.setAttribute("aria-pressed", mode==="cards" ? "true" : "false");
//   btnModeTable.setAttribute("aria-pressed", mode==="table" ? "true" : "false");
//   renderStep();
// }
// btnModeCards.addEventListener("click", ()=> setMode("cards"));
...
btnSubmit.addEventListener("click", ()=>{
  // final validate minimal
  if(!validateAll()) return;

  // mark submitted snapshot
...
function getAutoMode(){
  // samakan breakpoint dengan CSS kamu (820px / 980px). Pilih salah satu.
...
function renderStep(){
  ...
  void stepMount.offsetWidth; // restart animation
...
if(mode === "table"){
  return head + renderTable(key, list);
}
return head + renderCards(key, list);
}
  ...
  return `
    <div class="fhcTableWrap">
      <table class="fhcTable" style="min-width:0;">
        <thead>
          <tr><th>Item</th><th class="num">Jumlah (saat ini)</th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
...
function updateSubtotalOnly(stepKey){
  // update subtotal angka tanpa re-render seluruh step (biar kursor gak hilang)
  ...
  if(stepKey === "pemasukan") total = sumListMonthly(state.pemasukan);
    ...
    total = a - d; // yang tampil di step ini "kekayaan bersih"
  } else {
    return;
  }
...
function bindStepHandlers(stepKey){
  ...
  if(stepKey === "profil"){
    ...
    nama && nama.addEventListener("input", ()=>{ p.nama = nama.value; saveState(); });
    status && status.addEventListener("change", ()=>{ p.status = status.value; saveState(); });
    ...
    target && target.addEventListener("change", ()=>{ p.targetDanaDarurat = target.value; saveState(); });
    catatan && catatan.addEventListener("input", ()=>{ p.catatan = catatan.value; saveState(); });
    rencana && rencana.addEventListener("input", ()=>{ p.rencana12bulan = rencana.value; saveState(); });
  ...
  // amount inputs (FIX: jangan renderStep saat user mengetik)
document.querySelectorAll("input.money").forEach(inp=>{
  inp.addEventListener("input", ()=>{
    // saat mengetik: jangan format ribuan (biar caret stabil)
    ...
    // cukup update subtotal angka
    updateSubtotalOnly(stepKey);
  });

  inp.addEventListener("blur", ()=>{
    // saat selesai (blur): baru format ribuan
  ...
  document.querySelectorAll("[data-per]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      ...
      saveState();
      renderStep(); // to update pressed state + subtotal
    });
  });

  // period selects (table)
...
function addCustomItem(key){
  ...
  : "Item ditambahkan. Untuk ubah nama item, lakukan di tampilan desktop."
);
  renderStep();
}

function removeItem(key, idx){
  if(key === "assets" || key === "debts") return; // no remove for base snapshot
  const list = state[key];
...
function openReview(){
  ...
  const commM   = sumListMonthly(state.komitmen);
...
(function init(){
  // mode buttons
//   btnModeCards.setAttribute("aria-pressed", state.meta.mode==="cards" ? "true" : "false");
//   btnModeTable.setAttribute("aria-pressed", state.meta.mode==="table" ? "true" : "false");
